...
- **태그** — 제목 끝에 `#kipa #개인`처럼 붙이면 태그로 분리. 상세에서 편집, 목록의 태그 칩을 누르면 그 태그만, 대시보드도 태그별로. 태그마다 **색**이 이름 해시로 자동 배정됩니다(4색이라 태그가 많으면 겹칩니다 — 색은 묶음을 눈에 띄게 하는 보조 신호이고, 글자는 잉크색이라 어떤 색이든 읽힙니다)
